        ...
        <>
            <div className="flex justify-center">
                <div className="relative filter-bar inline-flex rounded-full dark:bg-white/30 bg-black/50 shadow-md">
                    ...
                    { categories? categories.map((element) => <button ref={el => refs.current[element.tag] = el} onClick={() => handleClick(element.tag)} className="rounded-full py-[8px] px-[16px] text-[22px] font-bold text-white bg-white/0 z-50 hover:opacity-75 active:opacity-50" key={element.tag}>{element.name}</button>) : "Nessuna categoria" }
                </div>
            </div>
        </>
    )
